...
AdsRequest = function (AdObj){
	...
	for (v in AdObj.schedule) {
		switch (AdObj.schedule[v].position) {
			case "pre-roll":
				var a = new AdSlot(i1,"pre-roll",0,AdObj.schedule[v].zone);
				i1++;
				AdList.push(a);
				break
			case "mid-roll":
...
function resumePlayBackAfterSlotShow(dataurl) {
	videoTag.removeEventListener('ended',resumePlayBackAfterSlotShow,false);
  $(".skipBtn").hide();
  Utils.showMessage({msgTxt:Msg.en_us.videoLoading,action:"wait"});
	videoTag.src = (!!videoTag.mainTrack)? videoTag.mainTrack :dataurl;
	videoTag.play();
